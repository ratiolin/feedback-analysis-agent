...
export default async function EvaluationPage() {
  ...
  return <><PageHead eyebrow="EVALUATION" title="先展示错误，再展示分数。" description="锁定合成校验集 N=60，仅用于机制评估；已完成 AI 辅助一致性复核，不构成独立人工审计。" />
    <section className="panel"><h2>v2 候选状态</h2><p>状态：{candidateState}；数据：{candidate.dataset_version ?? "unknown"}；候选提示词 SHA-256：<code>{candidate.candidate_prompt_sha256 ?? "unknown"}</code>。</p><p>{candidateProblemF1 === undefined ? "候选 DSL 尚未导入，v4 锁定集尚未运行模型评测；当前不得声称候选质量提升。" : `候选问题类型 Macro-F1：${candidateProblemF1.toFixed(3)}。`}</p></section>
    <h2>当前已发布 v1 基线</h2>
