...
四个工作流均已发布；2026-07-19 控制台与仓库 DSL 已同步为 `deepseek-v4-flash`，2026-08-01 完成 flash 真实回放与冻结评测：结构化工作流九项质量门全部通过（B³ F1 0.857、重复识别精确率 100%、首轮依赖成功率 100%）；内容工作流问题簇两次回放均 30/30，但候选 SOP（5/5 与 4/5）与周报叙事（5/6 与 6/6）存在偶发不合规输出（不可逆动作表述被门禁拦截），SOP 与周报提示词已强化、待控制台复测，内容套件保持未晋升。下述 V7 数据来自模型切换前的冻结版本，只作历史基线。
